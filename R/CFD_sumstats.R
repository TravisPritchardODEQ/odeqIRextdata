#' CFD_sumstats
#'
#' Calculates summary statistics from CFD continuous data templates, and creates AWQMS import files. Running function
#' will bring up a file selctor. Summary stats file will be saved at the same location.
#'
#' @import dplyr
#' @import lubridate
#' @import openxlsx
...
#' @param type How to select files. "file" lets you specify path in function arguments, 'file_select' brings up a chooser window.
...
# Error checking --------------------------------------------------------------------------------------------------
  ...
  if(!type %in% c("file", "file_select")){
    stop("type must be 'file' or 'file_select")
...
Results_import <- read_xlsx(filepath, sheet = "Results", col_types = results_col_types)

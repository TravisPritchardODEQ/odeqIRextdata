...
#' update somewhere (original dataset, data processing, etc..)
#'
...
dup_check <- df %>%
  mutate(join_start_date = ymd(.data[[date_col]]),
         join_start_time = ifelse(length(strsplit(.data[[time_col]], ":")[[1]][1]) == 1,
                                  paste0("0", strsplit(.data[[time_col]], ":")[[1]][1], ":", strsplit(.data[[time_col]], ":")[[1]][2]),
                                  .data[[time_col]]),
         join_depth = ifelse(.data[[depth_col]] == "", NA, .data[[depth_col]] ),
         join_Statistical_Base = case_when(.data[[stat_basis_col]] == 'Daily Maximum' ~ 'Maximum',
         ...
         join_mloc = .data[[mloc_col]],
         join_CharID = .data[[char_col]],
         join_method = .data[[method_col]]
         ) %>%
  left_join(AWQMS_2_join, by = c('join_mloc' = 'MLocID',
                                 'join_CharID' = 'Char_Name',
                                 'join_start_date' = 'SampleStartDate',
                                 'join_start_time' = 'SampleStartTime',
                                 'join_method' = 'Method_Code',
                                 'join_depth' = 'act_depth_height',
                                 'join_Statistical_Base' = 'Statistical_Base'))
